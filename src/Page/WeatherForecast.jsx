...
async function fetchForecast(endpoint) {
	...
	try {
		...
		data = await r.json()
		if (
			!_.has(data, 'properties.periods') ||
			!_.isArray(data.properties.periods)
		) {
...
const ForecastDayCard = ({
	...
	"
		>
			<span className="break-all hyphens-auto text-center text-xl text-neutral-600">
				{name}
			</span>
...
const WeatherForecast = (props) => {
	...
	const [location, setLocation] = useState('NY1')
	...
	return (
		<div className="flex flex-col items-center min-w-80">
			<div className="py-8 text-xl">
				<select
					className="mr-2 p-1 border border-neutral-300 rounded-md outline-neutral-600"
					value={location}
