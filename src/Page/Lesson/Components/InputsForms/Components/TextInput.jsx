...
const InternalTextInput = ({name, id, type='text', className, placeholder, value='', delay=false, onChange}) => {
	...
	if (_.isFunction(onChange)) {
		if (_.isInteger(delay)) {
			...
			internalOnChange = (e) => {setInternal(e.target.value); onChange(e.target.value)}
		}
	}

	return <input name={name} id={id} className={className}
		type={type} placeholder={placeholder} value={internal} onChange={internalOnChange}
	/>
}
...
const TextInput = (props) => {
	...
	if (_.isString(props.label)) {
		return  <label className={props.classNameLabel} htmlFor={id}>
