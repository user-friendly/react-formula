...
const Message = ({children, animate = false}) => {
	return (
		<div
			className={`border border-gray-300 px-6 py-3 m-2 rounded-full text-gray-600 ${animate ? 'animate-slideDown' : null}`}
		>
			{children}
		</div>
	)
	...
	return (
		<div className="flex flex-col items-center">
			...
			<div className="flex flex-col max-w-lg w-full">
				{history.map((m, i) => (
					<Message key={i} animate={false}>
						{m}
					</Message>
				))}
				{messages.map((m, i) => (
					<Message key={i} animate={true}>
						{m}
					</Message>
				))}
			</div>
