...
const deleteFurniture = (id) => {
	return fetch(`${ENDPOINT_URL}/${id}`, {
		...
		body: JSON.stringify({comment: `User requested record (${id}) to be removed from data store.`}),
	})
}
    ...
    <div className="flex m-8 border rounded-lg shadow-md border-stone-300 overflow-clip">
      ...
      <div className="flex flex-col w-full px-6 py-4 bg-white">
		...
		<div className="text-right">
			<button className="px-3 py-2 bg-rose-500 rounded-lg text-white text-md hover:bg-rose-600 active:bg-rose-700"
				onClick={(e) => onDelete(e, furniture.id)}>🗑️ Delete</button> 
		</div>
      </div>
    </div>
  )
...
const DeletingFurni = () => {
	...
	const furnitureItems = furnitures.map((furniture, idx) => (
	  <FurnitureItem furniture={furniture} onDelete={handleDelete} key={furniture.id} />
	))
	
	return 	(
	    <div className="flex justify-center">
	      <div className="w-full max-w-2xl">{furnitureItems.length ? furnitureItems : 'Store is out of furniture items!'}</div>
	    </div>
	  )
}
