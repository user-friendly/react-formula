...
const MessageBoard = () => {
	...
	return (
		<div className="flex flex-col items-center">
			<div ref={messagesRef} className="w-full max-w-xl flex flex-col max-h-[900px] overscroll-contain overflow-y-auto overflow-x-hidden">{messageItems}</div>
			<Form onSubmit={handleSubmit}
				className="w-full max-w-xl px-2 pr-4 flex items-center">
				
				<input name="username" value={user} onChange={e => setUser(e.target.value)} className="w-24 p-1 m-2 border border-gray-300 rounded-md" />
				<input ref={textInputRef} name="message" value={text} onChange={e => setText(e.target.value)} placeholder="enter message"
					autoComplete="off" className="flex-1 p-1 m-2 border border-gray-300 rounded-md" />
