...
const ListItem = ({children, data}) => {
	return (
		<div className="w-[22rem] m-2 flex flex-col">
			<div
...
export default ({children, displaySize = 3}) => {
	...
	return (
		<div className="flex flex-col items-center text-neutral-600">
			<div className="text-5xl border-b-4 border-orange-400 pb-3 mb-10">
				Customers Love Al's Autos
			</div>
			...
			<div className="flex items-center">
				<button
					className="text-4xl"
					onClick={(e) => (offset > 0 ? setOffset(offset - 1) : null)}
				>
					⬅
				</button>

				<div className="flex flex-wrap justify-center items-strech">
					{data.slice(offset, offset + displaySize).map((d, i) => (
				...
				<button
					className="text-4xl"
					onClick={(e) =>
						offset < data.length - displaySize ? setOffset(offset + 1) : null
					}
				>
					➡
				</button>
