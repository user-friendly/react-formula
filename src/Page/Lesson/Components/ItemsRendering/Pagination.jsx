...
const ListItem = ({children, data}) => {
	return (
		...
		>
			<span className="text-white bg-neutral-600/50 backdrop-blur-sm py-1 px-2 m-2 rounded-lg">
				👍 {data.likes}
			</span>
		</div>
	)
...
export default ({children, displaySize = 6}) => {
	...
	const paginator = (
		<Pagination
			...
			current={offset}
			pageSize={pageSize}
			onClick={(e, p) => setOffset(p * pageSize)}
		/>
	)
	...
	return (
		<div className="flex flex-col items-center text-neutral-600">
			<div className="text-5xl border-b-4 border-gray-400 pb-3 mb-7">
				Recent Posts
			</div>

			<div className="text-lg">
				<button
					className={`text-lg border rounded-md px-3 py-1 mx-1 ${pageSize == 3 ? selected : notSelected}`}
					onClick={(e) => updatePageSize(3)}
				>
					3
				</button>
				<button
					className={`text-lg border rounded-md px-3 py-1 mx-1 ${pageSize == 6 ? selected : notSelected}`}
					onClick={(e) => updatePageSize(6)}
				>
					6
				</button>
				<button
					className={`text-lg border rounded-md px-3 py-1 mx-1 ${pageSize == 9 ? selected : notSelected}`}
					onClick={(e) => updatePageSize(9)}
				>
					9
				</button>
			</div>
			...
			<div className="max-w-6xl flex flex-wrap justify-center">
				{data.slice(offset, offset + pageSize).map((d, i) => (
